...
class Stars {
  constructor(canvasId, maxRadius = 3, minRadius = 1, density = 'low') {
...
function initStarfield() {
  ...
  canvases.forEach((canvas) => {
    const maxRadius = canvas.getAttribute('data-max-radius')
      ? parseInt(canvas.getAttribute('data-max-radius'))
      : undefined
    const minRadius = canvas.getAttribute('data-min-radius')
      ? parseInt(canvas.getAttribute('data-min-radius'))
      : undefined
    const density = canvas.getAttribute('data-density') || undefined

    const randomId = generateRandomId()
    canvas.id = randomId
    new Stars(canvas.id, maxRadius, minRadius, density)
  })
}
